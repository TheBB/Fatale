@testset "Lagrange" begin
    ...
    @test basis(domain[1], @SVector [0.1]) ≈ [0.9, 0.1]
    @test basis(domain[1], @SVector [0.3]) ≈ [0.7, 0.3]
    ...
    @test basis(domain[1], @SVector [0.1]) ≈ [0.72, 0.36, -0.08]
    @test basis(domain[1], @SVector [0.3]) ≈ [0.28, 0.84, -0.12]
    ...
    @test basis(domain[1], @SVector [0.1]) ≈ [0.5355, 0.6885, -0.2835, 0.0595]
    @test basis(domain[1], @SVector [0.3]) ≈ [0.0385, 1.0395, -0.0945, 0.0165]
    ...
    basis = optimize(local_basis(domain, Lagrange, 3))
    @test basis(domain[1,1], @SVector [0.1, 0.1]) ≈ [
    ...
    @test basis(domain[1,1], @SVector [0.1, 0.3]) ≈ [
